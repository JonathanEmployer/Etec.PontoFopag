...
namespace cwkPontoMT.Integracao.Relogios.DIXI
{
    public class IDNOX : Relogio
    {
        ...
        private string EnviaDadosRep(string dados)
        {
            try
            {
                ...
                string retorno = Encoding.Default.GetString(bb.Take(k).ToArray());
                tcpClient.Close();
                return retorno;
            }
            catch (Exception e)
        ...
        private string RecebeMarcacoes(DateTime? inicio, DateTime? fim)
        {
            ...
            do
            {
                ...
                nsrInicial += step;
            } while (Convert.ToInt32(info) == step && nsrFinal >= (nsrInicial - step));
            return resultado;
        }
        ...
        private string RecebeMarcacoesNsr(int nsrInicio, int nsrFim)
        {
            ...
            try
            {
                ...
                nsrFinal = Convert.ToInt32(trataRetorno(EnviaDadosRep(dadosFinal), out info, out oper).Replace("\r\n", ""));
            }
            catch (Exception)
            {
                //caso de problemas de parâmetros incorretos pega todos os bilhetes do REP 
