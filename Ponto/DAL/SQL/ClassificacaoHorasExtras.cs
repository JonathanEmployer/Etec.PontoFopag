    ...
    public class ClassificacaoHorasExtras : DAL.SQL.DALBase, DAL.IClassificacaoHorasExtras
    {
        ...
        public DataTable GetHorasExtrasClassificadasCalculo(List<int> idsMarcacoes)
        {
            ...
            string aux = string.Format(@"
                   ...
                   sum(t.ClassificadasDiurnaMin) ClassificadasDiurnaMin,
                   sum(t.ClassificadasNoturnaMin) ClassificadasNoturnaMin,
            ...
            FROM
              (SELECT m.id IdMarcacao,
                      ...
                      che.Tipo,
                      CASE
                          WHEN che.Tipo = 0 THEN 'Quantidade'
                          ELSE 'Total'
                      END TipoDesc,
                      che.IdClassificacao,
                      c.Codigo ClassificacaoCodigo,
                      ...
                      CASE
                          WHEN (che.Tipo = 1
                                AND (dbo.FN_CONVHORA(m.horasextrasdiurna) > ISNULL(ct.totalDiurna, 0))) THEN (dbo.FN_CONVHORA(m.horasextrasdiurna) - ISNULL(ct.totalDiurna, 0))
                          ELSE dbo.FN_CONVHORA(che.QtdHoraClassificadaDiurna)
                      END ClassificadasDiurnaMin,
                      CASE
                          WHEN (che.Tipo = 1
                                AND dbo.FN_CONVHORA(m.horasextranoturna) > ISNULL(ct.totalNoturna, 0)) THEN (dbo.FN_CONVHORA(m.horasextranoturna) - ISNULL(ct.totalNoturna, 0))
                          ELSE dbo.FN_CONVHORA(che.QtdHoraClassificadaNoturna)
                      END ClassificadasNoturnaMin,
               ...
               LEFT JOIN
                 (SELECT sum(dbo.FN_CONVHORA(cheClass.qtdHoraClassificadaDiurna)) totalDiurna,
                         sum(dbo.FN_CONVHORA(cheClass.qtdHoraClassificadaNoturna)) totalNoturna,
                         cheClass.IdMarcacao
