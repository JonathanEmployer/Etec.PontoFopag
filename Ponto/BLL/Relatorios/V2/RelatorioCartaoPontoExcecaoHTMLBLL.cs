...
{
    public class RelatorioCartaoPontoExcecaoHTMLBLL : RelatorioBaseBLL, IRelatorioBLL
    {
        public RelatorioCartaoPontoExcecaoHTMLBLL(IRelatorioModel relatorioFiltro, Modelo.UsuarioPontoWeb usuario, ProgressBar progressBar) : base(relatorioFiltro, usuario, progressBar)
        {
            RelatorioCartaoPontoHTMLModel parms = ((RelatorioCartaoPontoHTMLModel)relatorioFiltro);
            parms.NomeArquivo = "Relatorio_Cartao_Ponto_Excecao_HTML" + parms.InicioPeriodo.ToString("ddMMyyyy") + "_" + parms.FimPeriodo.ToString("ddMMyyyy");
        }
        ...
        private IList<pxyCartaoPontoEmployer> GetDados()
        {
            ...
            IList<pxyCartaoPontoEmployer> cps = bllCartaoPonto.BuscaDadosRelatorioPontoExcecao(funcs, parms.InicioPeriodo, parms.FimPeriodo, _progressBar, parms.OrdemRelatorio, parms.quebraAuto);
            return cps;
        }
        ...
        protected ParametrosReportHTML ParametrosHTML()
        {
            return new ParametrosReportHTML()
            {
                ...
                ResourceName = "BLL.Relatorios.V2.cshtml.CartaoPontoExcecaoHtml.cshtml"
            };
        }
    }
}
