...
namespace BLL.Relatorios.V2
{
    public class RelatorioRegistrosBLL : RelatorioBaseBLL, IRelatorioBLL
    {
        ...
        protected override string GetRelatorioExcel()
        {
            ...
            var idsMarcacoes = new List<int>();
            PedacosRelatorio.ToList().ForEach(f => todos.Merge(f));
            foreach (DataRow r in todos.Rows)
            {
                idsMarcacoes.Add(Convert.ToInt32(r["id"]));
            }

            BLL.MarcacaoHorasExtrasPercentual bllMarcHorasExtrasPercent = new MarcacaoHorasExtrasPercentual(_usuario.ConnectionString, _usuario);
            List<HoraExtraFuncionarioDia> horasExtrasDoPeriodo = bllMarcHorasExtrasPercent.CalculaHoraExtraPorIdsMarcacao(idsMarcacoes);
            ...
            Parallel.ForEach(PedacosRelatorio,
                dtMarcacoes =>
                {
                    try
                    {
                        ...
                        var TotalHoras = horasExtrasDoPeriodo.GroupBy(l => l.Percentual)
                                        .Select(lg =>
                                            new
                                            {
                                                Percentual = lg.Key
                        ...
                        foreach (DataRow dr in dtMarcacoes.Rows) // Adiciona os valores nos percentuais
                        {
                           ...
                           List<HoraExtraFuncionarioDia> HEDiaFuncionario = horasExtrasDoPeriodo.Where(x => x.IdFuncionario == idFuncionario && x.DataMarcacao == dataMarc).ToList();
