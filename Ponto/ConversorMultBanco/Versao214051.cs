...
{
    public class Versao214051
    {
        public static void Converter(DataBase db)
        {
            db.ExecuteNonQuery(CommandType.Text, TRG_01_DROP, null);
            db.ExecuteNonQuery(CommandType.Text, TRG_01, null);
        }

        private static readonly string TRG_01_DROP = @"
        IF OBJECTPROPERTY(OBJECT_ID('TRG_Marcacao'), 'IsTrigger') = 1
        BEGIN
            DROP TRIGGER [dbo].[TRG_Marcacao]
        END";

        private static readonly string TRG_01 = @"
        CREATE TRIGGER [dbo].[TRG_Marcacao] ON [dbo].[marcacao]
                FOR INSERT, UPDATE
                AS
                BEGIN
                  ...
                  begin
                    ...
                    BEGIN
                      SET @msgerror = 'Problemas ao gerar marcações, existem duplicidades. Por favor verifique se existe outro processo sendo executado, caso existe aguarde o termino e tente novamente! Erro encontrado em: id funcionario: '+ CONVERT(VARCHAR,@idFuncionario)+', Funcionario: '+(select convert(varchar,codigo)+' - '+nome from funcionario as f where f.id = @idFuncionario)+', Data: '+CONVERT(VARCHAR,@data,103) +
