...
namespace ConversorMultBanco
{
    public class Versao309009
    {
        public static void Converter(DataBase db)
        {
            ...
            db.ExecuteNonQuery(CommandType.Text, ALTER_TBL_CW_USUARIO_01, null);
            db.ExecuteNonQuery(CommandType.Text, ALTER_TBL_CW_USUARIO_02, null);
            db.ExecuteNonQuery(CommandType.Text, ALTER_TBL_CW_USUARIO_03, null);
            db.ExecuteNonQuery(CommandType.Text, ALTER_TBL_CW_USUARIO_04, null);
            db.ExecuteNonQuery(CommandType.Text, ALTER_TBL_CW_USUARIO_05, null);
            db.ExecuteNonQuery(CommandType.Text, ALTER_TBL_EMPRESA, null);
            db.ExecuteNonQuery(CommandType.Text, FN_01_DROP, null);
            db.ExecuteNonQuery(CommandType.Text, FN_01, null);
        }

        #region Alters Cw_acesso
        private static readonly string ALTER_TBL_01 = @"alter table funcao alter column descricao varchar(100) not null;";
        private static readonly string ALTER_TBL_02 = @"
            IF NOT EXISTS (	SELECT tabela.name as tabela, st.* FROM sys.all_columns st
				            ...
				            WHERE st.name = N'Controller' AND tabela.name = N'cw_acesso')
		            BEGIN
                        ALTER TABLE dbo.cw_acesso ADD
	                        Controller varchar(100) NULL;
		            END";
        private static readonly string ALTER_TBL_03 = @"
            IF NOT EXISTS (	SELECT tabela.name as tabela, st.* FROM sys.all_columns st
				            inner join sys.sysobjects tabela on tabela.id = st.object_id 
				            WHERE st.name = N'Nome' AND tabela.name = N'cw_acesso')
		            BEGIN
                        ALTER TABLE dbo.cw_acesso ADD
        ...
        private static readonly string ALTER_TBL_04 = @"
            IF NOT EXISTS (	SELECT tabela.name as tabela, st.* FROM sys.all_columns st
				            inner join sys.sysobjects tabela on tabela.id = st.object_id 
				            WHERE st.name = N'Menu' AND tabela.name = N'cw_acesso')
		            BEGIN
                        ALTER TABLE dbo.cw_acesso ADD
        ...
        private static readonly string ALTER_TBL_05 = @"
            IF NOT EXISTS (	SELECT tabela.name as tabela, st.* FROM sys.all_columns st
				            inner join sys.sysobjects tabela on tabela.id = st.object_id 
				            WHERE st.name = N'Consultar' AND tabela.name = N'cw_acesso')
		            BEGIN
                        ALTER TABLE dbo.cw_acesso ADD
        ...
        private static readonly string ALTER_TBL_06 = @"
            IF NOT EXISTS (	SELECT tabela.name as tabela, st.* FROM sys.all_columns st
				            inner join sys.sysobjects tabela on tabela.id = st.object_id 
				            WHERE st.name = N'Excluir' AND tabela.name = N'cw_acesso')
		            BEGIN
                        ALTER TABLE dbo.cw_acesso ADD
        ...
        private static readonly string ALTER_TBL_07 = @"
            IF NOT EXISTS (	SELECT tabela.name as tabela, st.* FROM sys.all_columns st
				            inner join sys.sysobjects tabela on tabela.id = st.object_id 
				            WHERE st.name = N'Cadastrar' AND tabela.name = N'cw_acesso')
		            BEGIN
                        ALTER TABLE dbo.cw_acesso ADD
        ...
        private static readonly string ALTER_TBL_08 = @"
            IF NOT EXISTS (	SELECT tabela.name as tabela, st.* FROM sys.all_columns st
				            inner join sys.sysobjects tabela on tabela.id = st.object_id 
				            WHERE st.name = N'Alterar' AND tabela.name = N'cw_acesso')
		            BEGIN
                        ALTER TABLE dbo.cw_acesso ADD
        ...
        #endregion
        #region Alters Cw_usuario
        private static readonly string ALTER_TBL_CW_USUARIO_01 = @"
            IF NOT EXISTS (	SELECT tabela.name as tabela, st.* FROM sys.all_columns st
				            inner join sys.sysobjects tabela on tabela.id = st.object_id 
				            WHERE st.name = N'PasswordSalt' AND tabela.name = N'cw_usuario')
		            BEGIN
                        ALTER TABLE dbo.cw_usuario ADD
	                        PasswordSalt varchar(200) NULL;
		            END";
        private static readonly string ALTER_TBL_CW_USUARIO_02 = @"
            IF NOT EXISTS (	SELECT tabela.name as tabela, st.* FROM sys.all_columns st
				            inner join sys.sysobjects tabela on tabela.id = st.object_id 
				            WHERE st.name = N'Password' AND tabela.name = N'cw_usuario')
		            BEGIN
                        ALTER TABLE dbo.cw_usuario ADD
	                        Password varchar(200) NULL;
		            END";
        private static readonly string ALTER_TBL_CW_USUARIO_03 = @"
            IF NOT EXISTS (	SELECT tabela.name as tabela, st.* FROM sys.all_columns st
				            inner join sys.sysobjects tabela on tabela.id = st.object_id 
				            WHERE st.name = N'UltimoAcesso' AND tabela.name = N'cw_usuario')
		            BEGIN
                        ALTER TABLE dbo.cw_usuario ADD
	                        UltimoAcesso datetime NULL;
		            END";
        private static readonly string ALTER_TBL_CW_USUARIO_04 = @"
            IF NOT EXISTS (	SELECT tabela.name as tabela, st.* FROM sys.all_columns st
				            inner join sys.sysobjects tabela on tabela.id = st.object_id 
				            WHERE st.name = N'connectionString' AND tabela.name = N'cw_usuario')
		            BEGIN
                        ALTER TABLE dbo.cw_usuario ADD
	                        connectionString varchar(400) NULL;
		            END";
        private static readonly string ALTER_TBL_CW_USUARIO_05 = @"
            IF NOT EXISTS (	SELECT tabela.name as tabela, st.* FROM sys.all_columns st
				            inner join sys.sysobjects tabela on tabela.id = st.object_id 
				            WHERE st.name = N'idUsuarioCentralCliente' AND tabela.name = N'cw_usuario')
		            BEGIN
                        ALTER TABLE dbo.cw_usuario ADD
	                        idUsuarioCentralCliente int NULL;
		            END";
        
        #endregion

        private static readonly string ALTER_TBL_EMPRESA = @"
            IF NOT EXISTS (	SELECT tabela.name as tabela, st.* FROM sys.all_columns st
				            inner join sys.sysobjects tabela on tabela.id = st.object_id 
				            WHERE st.name = N'InstanciaBD' AND tabela.name = N'Empresa')
		            BEGIN
                        ALTER TABLE dbo.Empresa ADD
	                        InstanciaBD varchar(100) NULL;
		            END";
